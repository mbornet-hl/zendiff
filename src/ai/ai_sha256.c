 ...
 *   File         :     ai_sha256.c
 *
 *   @(#)  [Zen] ai_sha256.c Version 1.4 of 23/09/13 - 
 *
...
void ai_sha256_process( ai_sha256_context *ctx, uint8 data[64] )
{
...
void ai_sha256_update( ai_sha256_context *ctx, uint8 *input, uint32 length )
{
    ...
    if( left && length >= fill )
    {
        memcpy( (void *) (ctx->buffer + left),
                (void *) input, fill );
        ai_sha256_process( ctx, ctx->buffer );
        length -= fill;
        input  += fill;
        left = 0;
    }
...
static uint8 ai_sha256_padding[64] =
{
...
void ai_sha256_finish( ai_sha256_context *ctx, uint8 digest[32] )
{
    ...
    ai_sha256_update( ctx, ai_sha256_padding, padn );
    ai_sha256_update( ctx, msglen, 8 );
