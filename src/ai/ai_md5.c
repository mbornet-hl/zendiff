 ...
 *   File         :     ai_md5.c
 *
 *   @(#)  [Zen] ai_md5.c Version 1.5 of 23/09/13 - 
 *
...
void ai_md5_process(ai_md5_context *ctx, uint8 data[64])
{
    uint32 X[16], A, B, C, D;
...
//fprintf(stderr, "INPUT = 0x%08X  length = %5lu\n", (unsigned int) input, length);
     if (! length) return;
...
static uint8 ai_md5_padding[64] =
{
...
void ai_md5_finish(ai_md5_context *ctx, uint8 digest[16])
{
    ...
    ai_md5_update(ctx, ai_md5_padding, padn);
    ai_md5_update(ctx, msglen, 8);
